  ...
  if (canWrite) {
    ...
    server.registerTool('create_alert', {
      ...
      inputSchema: {
        ...
        interval_seconds: z.number().int().min(30).optional().describe('Check interval in seconds (min 30, default 60)'),
        cooldown_seconds: z.number().int().min(0).optional().describe('Min seconds between repeated alerts (default 300)'),
